...
//		}
		...
		if (tb != null) dw.writeBytes(tb);
	}
	
	private static void write(DOSH dh, IDataWriter dw) throws IOException {
		byte[] bytes = dh.array();
		// DEBUG byte[] builder = new byte[dh.sizeOf()];
		dw.writeBytes(bytes);
		
//		ByteBuffer buffer = (ByteBuffer.wrap(builder));
//		buffer.order(ByteOrder.LITTLE_ENDIAN);
//		
//		for (Datum o : dh.values()) {
//			
//			int v = o.valueOf();
//			buffer.putShort((short) (v));
//			
//			//buffer.put((byte) (v & 0xff));
//			//buffer.put((byte) (v >> 8 & 0xffff));
//			//buffer.putInt(v);
//			System.err.print(v + ":");
//			System.err.print((byte) (v & 0xff) + ":");
//			System.err.print((byte) (v >> 8 & 0xff) + "\n");
//		}
	...
	private static void write(PESig s, IDataWriter dw) throws IOException {
		dw.writeBytes(s.signature.bytes);
	}
	
	private static void write(COFFH ch, IDataWriter dw) throws IOException {
//		dw.writeWord(ch.machine);
//		dw.writeWord(ch.numberOfSections);
//		dw.writeDoubleWord(ch.timeDateStamp);
//		dw.writeDoubleWord(ch.pointerToSymbolTable);
//		dw.writeDoubleWord(ch.numberOfSymbols);
//		dw.writeWord(ch.sizeOfOptionalHeader);
//		dw.writeWord(ch.characteristics);
	}
	
	private static void write(OHS oh, IDataWriter dw) throws IOException {
...
//		if (!is64) dw.writeDoubleWord(oh.getBaseOfData());
//		
...
//		if (entry.index != ImageDataDirectoryType.CERTIFICATE_TABLE) prd = rvc
//					.convertVirtualAddressToRawDataPointer(idd.getVirtualAddress());
//		if (prd > dw.getPosition()) {
//			byte[] pa = pe.getImageData().getPreamble(entry.index);
//			if (pa != null) dw.writeBytes(pa);
//			else dw.writeByte(0, prd - dw.getPosition());
//		}
//		
...
//		}
	...
	private static void writeDebugRawData(PE pe, DataEntry entry, IDataWriter dw)
		...
		if (entry.pointer > dw.getPosition()) {
			...
			if (pa != null) dw.writeBytes(pa);
			else dw.writeByte(0, entry.pointer - dw.getPosition());
		}
...
//		dw.writeBytes(b);
	...
	private static void write(PE pe, BoundImportDir bidt, IDataWriter dw)
				throws IOException {
...
//			if (!names.contains(s)) dw.writeUtf(s);
//			names.add(s);
//		}
//		
...
//					.getDataDirectory(ImageDataDirectoryType.BOUND_IMPORT).getSize();
//		if (bis > dpos) {
